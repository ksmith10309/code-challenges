...
const forEachTwoToThe = (arr) => {
  const newArr = [];
  arr.forEach(el => newArr.push(Math.pow(2, el)));
  return newArr;
}
...
const evenOdd = (arr) => {
  return arr.map(el => (isNaN(el)) ? 'N/A' : (el % 2 === 0) ? 'even' : 'odd');
}
...
const extractStats = (arr) => {
  return arr.map(obj => { return { name: obj.stat.name, total: obj.effort + obj.baseStat } });
}
