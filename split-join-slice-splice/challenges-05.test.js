...
const listFoods = (recipe) => {
  return recipe.ingredients.map(el => el.slice(el.indexOf(' ', el.indexOf(' ') + 1) + 1));
}
...
const splitFoods = (recipe) => {
  return recipe.ingredients.map(el => el.split(' ').slice(2).join(' '));
}
...
const stepActions = (recipe) => {
  return recipe.steps.map(el => el.split(' ')[0]);
}
...
const removeVowels = (str) => {
  let regex = /[aeiou]/i;
  return str.split('').filter(el => !regex.test(el)).join('');
}
...
const extractVowels = (str) => {
  let regex1 = /[aeiou]/i;
  let regex2 = /[aeiou]/gi;
  return [str.split('').filter(el => !regex1.test(el)).join(''), str.match(regex2).sort().join('')];
}
