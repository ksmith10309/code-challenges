...
const count = (target, input) => {
  return input.reduce((acc, cur) => acc.concat(cur), []).filter(el => el === target).length;
}
...
const totalSum = (input) => {
  return input.reduce((acc, cur) => acc.concat(cur), []).reduce((acc, cur) => acc + cur, 0);
}
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map(arr => arr.filter(el => typeof el === 'number' && el % 5 === 0)
...
let findMaleAndFemale = (data) => {
  return data.filter(obj => obj.gender === 'male' || obj.gender === 'female')
             .reduce((acc, cur, idx, arr) => {
               return (idx === arr.length - 1) ? acc + cur.name : acc + cur.name + ' and ';
             }, '');
}
...
let findShortest = (data) => {
  return data.reduce((acc, cur) => (parseInt(cur.height) < parseInt(acc.height)) ? cur : acc).name;
}
