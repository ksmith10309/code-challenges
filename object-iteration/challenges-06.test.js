...
const hasChildrenValues = (arr, character) => {
  return Object.values(arr.find(obj => obj.name === character))[2].length > 0;
}
...
const hasChildrenEntries = (arr, character) => {
  return Object.entries(arr.find(obj => obj.name === character))[2][1].length > 0;
}
...
const houseSize = (arr) => {
  const sizes = arr.map(obj => Object.values(obj).reduce((acc, cur) => acc.concat(cur), [])
                                                 .reduce((acc, cur, idx, arr) => {
                                                   if (idx === arr.length - 1) acc.house = cur;
                                                   else if (cur !== null) acc.members++;
                                                   return acc;
                                                 }, { members: 0 }));
  return sizes;
}
...
const houseSurvivors = (arr) => {
  const survivors = arr.map(obj => Object.values(obj).reduce((acc, cur) => acc.concat(cur), [])
                                                     .reduce((acc, cur, idx, arr) => {
                                                       if (idx === arr.length - 1) acc.house = cur;
                                                       else if (cur !== null && !deceasedSpouses.includes(cur)) acc.members++;
                                                       return acc;
                                                     }, { members: 0 }));
  return survivors;
}
