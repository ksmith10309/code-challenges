...
const countNumberOfChildren = (arr) => {
  return arr.reduce((acc, cur) => acc + (cur.children || []).length, 0);
}
...
const extractStat = (statName, arr) => {
  return arr.reduce((acc, cur) => (cur.stat.name === statName) ? cur : acc, null);
}
...
const extractChildren = (arr) => {
  const regex = /[a]/i;
  return arr.filter(obj => regex.test(obj.name)).reduce((acc, cur) => acc.concat(cur.children || []), []);
}
